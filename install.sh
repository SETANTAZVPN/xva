...
VALIDITY () {
    ...
    Exp1=$(curl https://raw.githubusercontent.com/SETANTAZ/ip/main/ip | grep $MYIP | awk '{print $4}')
    if [[ $today < $Exp1 ]]; then
...
IZIN=$(curl https://raw.githubusercontent.com/SETANTAZ/ip/main/ip | awk '{print $5}' | grep $MYIP)
if [ $MYIP = $IZIN ]; then
...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://github.com/SETANTAZ/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
# validity
rm -f /usr/bin/e
valid=$(curl https://github.com/SETANTAZ/ip | grep $MYIP | awk '{print $4}')
...
today=`date -d "0 days" +"%Y-%m-%d"`
Exp1=$(curl https://github.com/SETANTAZ/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
# REPO    
    REPO="https://github.com/SETANTAZ/xva/"
...
function password_default() {
    ...
    CHATID="2105857557"
    KEY="5755909226:AAFM9zBAk8nVZzl4zTINU1SakYULxH0_P0U"
    TIME="10"
